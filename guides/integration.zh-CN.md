...
命令会显示二维码；请在 Yeelight Pro APP 首页点击右上角 `+` -> **MCP 授权**扫码。单家庭自动选择，多个家庭时才按名称询问。Cloud 模式会同时配置 Metadata 与 IoT MCP；手动 Token 配置仅作为高级兼容路径。
...
客户端 URL 配置为 `http://127.0.0.1:9000/mcp`。本地服务和远程服务一样，每个请求都必须显式发送 Authorization Header；服务不会读取操作者电脑上的 CLI Profile。测试凭据只能通过显式隔离注入提供。
